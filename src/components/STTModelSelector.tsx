    ...
    <div className="space-y-3">
      ...
      <div className="relative">
        <button
          ...
          className="w-full px-4 py-2.5 bg-[var(--surface-elevated)] border border-[var(--border-soft)] rounded-lg text-left flex items-center justify-between hover:border-[var(--border-strong)] focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
          <div className="flex items-center gap-2">
            <span className="text-xl">{selectedModelInfo?.icon}</span>
            <span className="text-[var(--text-strong)]">{selectedModelInfo?.name || 'Select model'}</span>
          </div>
          <svg
        ...
        {/* Dropdown menu */}
        {isOpen && (
          <div className="absolute z-10 mt-1 w-full bg-[var(--surface-elevated)] border border-[var(--border-soft)] rounded-lg shadow-lg max-h-80 overflow-y-auto">
            {MODELS.map((model) => {
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${!isSupported
                      ? 'bg-[var(--surface-muted)] opacity-60 cursor-not-allowed'
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className={`font-medium ${!isSupported ? 'text-[var(--text-soft)]' : 'text-[var(--text-strong)]'}`}>
                            {model.name}
                          </span>
                          {model.isNew && (
                            <span className="text-xs font-semibold uppercase tracking-wide text-emerald-500 bg-[var(--surface-elevated)] px-2 py-0.5 rounded border border-[var(--border-soft)]">
                              New
                            </span>
                          )}
                          {!isSupported && (
                        ...
                        <p className={`text-xs ${!isSupported ? 'text-[var(--text-soft)]' : 'text-[var(--text-muted)]'}`}>
                          {model.description}
                        </p>
                      </div>
                    </div>
      ...
      {/* Selected model card */}
      {selectedModelInfo && (
        <div className="p-4 rounded-lg border-2 border-blue-500 bg-[rgba(59,130,246,0.14)]">
          <div className="flex items-start gap-3">
            <span className="text-2xl flex-shrink-0">{selectedModelInfo.icon}</span>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="font-semibold text-[var(--text-strong)]">{selectedModelInfo.name}</h3>
              ...
              <p className="text-sm mt-1 text-[var(--text-body)]">{selectedModelInfo.description}</p>
              <p className="text-xs mt-1 text-[var(--text-muted)]">{selectedModelInfo.useCases}</p>
              <div className="mt-2 flex flex-wrap gap-1">
                {selectedModelInfo.features.map((feature, idx) => (
                  <span
                    key={idx}
                    className="inline-block px-2 py-0.5 text-xs rounded bg-[var(--surface-elevated)] text-[var(--text-body)]"
                  >
                    {feature}
                  </span>
                ))}
